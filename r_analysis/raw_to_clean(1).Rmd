...
```{r}
cse_summary_table = cse_summary_table %>%
  mutate(contingency = str_remove(block_feedback_type, "(no_punishment_)|(punishment_)|(random_punishment)")) %>% 
  mutate(condition = str_remove(block_feedback_type, "(_congruent)|(_incongruent)|(_random)"))
```
